...
+(NSString*) getData:(NSData*) data{
    
    Byte *testByte = (Byte *)[data bytes];
    Byte bytes[data.length -4];
    for(int i= 4;i<[data length]  ;i++){

        bytes[i] = testByte[i];
     
           //NSLog(@"array=%s",testByte[i]);
  
    }
    
   NSData*  d2 =  [[NSData alloc]initWithBytes:bytes length:data.length -4];
    NSString*  str = [[NSString alloc] initWithData:d2 encoding:NSUTF8StringEncoding] ;
    NSLog(@"%@",str);
    
    //bytes[data.length] = 0;
    
    
 //  NSData *adata = [[NSData alloc] initWithBytes:bytes length:data.length-1];
 
      //  [NSString alloc]initWithData:unicodeStr encoding: NSUTF8StringEncoding)
//        NSString *tempStr1 = [unicodeStr stringByReplacingOccurrencesOfString:@"\\x"withString:@"\\x"];
...
//                                                               errorDescription:NULL];
   ...
   // NSData *data = NSSting.init
//    NSError *error;
//    id jsonObject = [NSJSONSerialization JSONObjectWithData:unicodeStr options:0 error:&error];
//    
//     [unicodeStr dataUsingEncoding:NSUTF8StringEncoding]
//    
//    printf("@",jsonObject);
    
 //  NSString *matchedString1 = [unicodeStr stringByMatching:regexString capture:1L];
